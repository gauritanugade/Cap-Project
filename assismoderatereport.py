from flask import Flask,request,redirect,url_for,flash,render_template,Response,session,Blueprint
...
assismoderatereport = Blueprint('assismoderatereport', __name__, template_folder='templates')

app=assismoderatereport
...
@app.route('/billing')
def home():
    return render_template('assismoderatebilling.html')
...
def generate_bill():
    if request.method == "POST":
        ...
        cursor.execute("select DISTINCT faculty.faculty from issue_check  JOIN time_table ON time_table.timetable_id = issue_check.timetable_id JOIN faculty ON time_table.faculty_id = faculty.faculty_id  JOIN subject ON time_table.subject_id = subject.subject_id JOIN coursename ON time_table.coursename_id = coursename.coursename_id JOIN examsession ON time_table.examsession_id = examsession.examsession_id inner join teacher on issue_check.teacher_id=teacher.teacher_id inner join paper_count on paper_count.papercount_id=issue_check.papercount_id where teacher.teacher_name=%s and issue_check.return_date is not null",(teacher_name,))
        facultyname = cursor.fetchall()
        ...
        totalaccesrupees=0
        sumtotal=0
        cursor.execute("select issue_check.teacher_paper_count,faculty.duration from issue_check  JOIN time_table ON time_table.timetable_id = issue_check.timetable_id JOIN faculty ON time_table.faculty_id = faculty.faculty_id  JOIN subject ON time_table.subject_id = subject.subject_id JOIN coursename ON time_table.coursename_id = coursename.coursename_id JOIN examsession ON time_table.examsession_id = examsession.examsession_id inner join teacher on issue_check.teacher_id=teacher.teacher_id inner join paper_count on paper_count.papercount_id=issue_check.papercount_id where teacher.teacher_name=%s and issue_check.return_date is not null",(teacher_name,))
        ...
        for total in totals:
            ...
            try:
                if total != None and len(total)>0:
                    ...
                    sumtotal=sumtotal+total_integer
                    ...
                    if duration==2:
                        assesrupees=total_integer*9 

                    elif assesrupees==None:
                    ...
                    totalaccesrupees=totalaccesrupees+assesrupees
        ...
        for total1 in totals1:
            try:
                if total1 != None and len(total1) > 0:
                    ...
                    totalrupees=totalrupees+modrupees
        ...
        totalpapersum=sumtotal+sumtotal1
        ...
        faculty_values = "/".join(str( facultyname[0]) for  facultyname in  facultyname)
        ...
        pdf.cell(40,th,str(sumtotal),border=1)
        pdf.cell(45,th,str(sumtotal1),border=1)
        pdf.ln()
        pdf.cell(109.2,th,"*Total No .Of Answer Books Assessed And Moderated",border=1)
        pdf.cell(85,th,str(totalpapersum),border=1)
        pdf.ln(8)

...
def home():
    ...
    print("list:", joblist2)
    cursor.close()
    return render_template("time.html", joblist=joblist, joblist1=joblist1, joblist2=joblist2)
...
def faculty():
    ...
    print("degree:", degree)
    cursor = mysql.connection.cursor()
    queryselect = 'select DISTINCT cap_project.faculty.faculty,cap_project.examsession.degree,cap_project.faculty.faculty_id from cap_project.faculty inner join cap_project.examsession on cap_project.faculty.faculty_id=cap_project.examsession.faculty_id where cap_project.examsession.degree=%s'
...
def course():
    ...
    year = request.form["year"]  # Corrected variable name
